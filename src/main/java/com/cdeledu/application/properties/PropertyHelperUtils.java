package com.cdeledu.application.properties;
...
import com.cdeledu.application.system.JavaRuntimeInfo;
...
public class PropertyHelperUtils {
	...
	public static Properties getProps(String filePath) {
		...
		try {
			fileUrl = ConfigUtil.class.getClassLoader().getResource(filePath);
			...
			if (ist == null)
				ist = ConfigUtil.class.getResourceAsStream(filePath);
	 ...
	 * 
	 * @Title: changeValue
	 * @Description: 修改properties 的键值对的值
	 * @author: 独泪了无痕
	 ...
	 * @return
	 */
	public static boolean changeValue(String property, String name, String value) {
		boolean flag = false;
		try {
			String filePath = JavaRuntimeInfo.getClassParentPath() + property;
			Properties props = getProps(property);
			FileOutputStream file = new FileOutputStream(filePath);
			// 设置属性值，如不属性不存在新建
			props.setProperty(name, value);
			// 重新写入配置文件
			props.store(file, filePath);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return flag;
