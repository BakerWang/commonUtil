package com.cdeledu.webCrawler.crawlerCity;
...
public class crawlerCityHelper {
		...
		for (Element ele1 : eles1) {
			...
			for (Element ele2 : eles2) {
				...
				for (Element ele3 : eles3) {
					...
					Document doc3 = WebCrawler.getDocument(eles4.get(1).absUrl("href"));
					// 代码
					eles4.get(0).text();
					// 名称
					eles4.get(1).text();
					...
					for (Element ele5 : eles5) {
						...
						// 名称
						eles6.get(1).text();
						Document doc4 = WebCrawler.getDocument(eles6.get(1).absUrl("href"));
						...
						for (Element ele7 : eles7) {
							...
							// 名称
							eles8.get(1).text();
							Document doc5 = WebCrawler.getDocument(eles8.get(1).absUrl("href"));
	...
	public static void webCrawler() {
		...
		Document document = WebCrawler.getDocument(crawlerUrl);// 得到的document一定是正常的
